...
export class PriceHistoricalController
{
    ...
    async getPriceWithIndicators(@Param('tokenPair') tokenPair: string, @Param('interval') interval: string, @Param('year') year: number, @Param('month') month: number) : Promise<PriceKlineWithIndicatorsModel[]>
    {
